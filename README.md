...
本项目以电脑端浏览器为主要使用场景。由于三表分析需要上传和校对资产负债表、损益表、现金流量表，手机端操作并不方便，因此暂不做专门的移动端适配。

手机浏览器可以直接打开电脑端网页并输入激活码，但推荐客户在电脑端完成正式上传、校对、分析和报告导出。激活码默认支持 2 个同时在线会话，方便客户先在手机端试用后，再在电脑端继续使用同一个激活码。
